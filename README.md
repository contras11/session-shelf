...
- タイトル・概要・プロジェクト名と、会話本文によるローカル検索
...
- ompのプロジェクト名はセッションディレクトリ名から復号し、ログ本文の`cwd`は使いません。ハイフンはフォルダ名にも区切りにもなるため、各階層で実在する最長のフォルダ名を選び、実在しないパスだけ単純な置換に戻します。`parentId`はファイル内のレコード連鎖で、親セッションとしては扱いません。`agent.db`、`config.yml`、`history.db`、`logs`は読みません。
...
- シンボリックリンク、更新から30分以内の項目、用途を確認できない項目は保護します。Claude Codeは`~/.claude/sessions`の記録、ompは`~/.omp/run/daemons`の生存プロセスとプロジェクトが一致するセッションを、実行中として保護します。
...
- Claude Codeまたはompで、プロセスが生存している実行中のセッション
